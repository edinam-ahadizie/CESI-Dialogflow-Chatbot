...
const mapReplies = (msg: any) =>
  (msg.replies || []).map(reply => ({ type: 'reply', value: reply }));
...
export class Agent {
  ...
  constructor(private user: User) {
    this.client = new ApiAiClient({
      accessToken: '5d9957756dd34cb7ba6f7ffd6cc1b749'
    });
  ...
  private onResponse(response: any): void {
    ...
    ff.messages
      ...
      .forEach(msg => suggestedActions.push(...mapReplies(msg)));
